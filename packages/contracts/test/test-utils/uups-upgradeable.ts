...
export async function deployAndUpgradeSelfCheck(
  ...
  const deployerSigner = (await hre.ethers.getSigners())[deployerIdx];
  const upgraderSigner = (await hre.ethers.getSigners())[upgraderIdx];

  const proxy = await hre.wrapper.deployProxy(deployerIdx, fromArtifact, {
  ...
  if (managingContract === undefined) {
    await expect(
      hre.wrapper.upgradeProxy(upgraderIdx, proxy.address, toArtifact, {
        args: Object.values(args),
      })
    )
  ...
  const toImplementation = (await hre.wrapper.deploy(toArtifact)).address;
...
export async function deployAndUpgradeFromToCheck(
  ...
  let proxy = await hre.wrapper.deployProxy(deployerIdx, fromArtifact, {
    args: Object.values(args),
    initArgs: Object.values(initArgs),
    withProxy: true,
    proxySettings: {
  ...
  proxy = await hre.wrapper.upgradeProxy(
    upgraderIdx,
    proxy.address,
    toArtifact,
    {
