...
find "$SOURCE_DIR" -maxdepth 1 -name '*.json' | sort | while read source_file; do
    ...
    # Extract the address
    value=$(jq -er '.["v1.4.0"].MultisigRepoProxy.address // .["v1.3.0"].MultisigRepoProxy.address // empty' "$source_file")
    jq_exit_code=$?

    if [[ $jq_exit_code -ne 0 || "$value" == "null" ]]; then
        echo "  Warning: Could not find 'MultisigRepoProxy' under 'v1.4.0' or 'v1.3.0' in '$filename'. Skipping." >&2
        continue
    fi
